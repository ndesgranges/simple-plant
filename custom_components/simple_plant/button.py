...
async def async_setup_entry(
    ...
    entities: list[SimplePlantButton] = [
        SimplePlantButton(hass, entry, desc) for desc in ENTITY_DESCRIPTIONS
    ]
    if entry.data.get("days_between_fertilizations"):
        entities.extend(
            SimplePlantFertilizeButton(hass, entry, desc)
...
class SimplePlantFertilizeButton(ButtonEntity):  # pylint: disable=abstract-method
    """simple_plant fertilize button class."""
    ...
    def __init__(
        ...
        """Initialize the fertilize button class."""
        super().__init__()
